...
with open('svmClassifier_pvf.pkl', 'rb') as file:
    clf = pickle.load(file)

def callback(data):
    gesturePred.data = clf.predict(np.array(data.data)[np.newaxis]) # take from the message called gesturePred only the data (not the header) and predicts pose and reformats it
